...
public class AuthController implements AuthControllerOpenApi {
	...
	public ResponseEntity<AuthResponseDTO> refresh(HttpServletRequest request) {
		...
		return ResponseEntity.ok()
				...
				.body(new AuthResponseDTO("Sessao renovada com sucesso.", null));
	}
	...
	public ResponseEntity<Void> logout(HttpServletRequest request) {
		...
		return ResponseEntity.noContent()
				.header(HttpHeaders.SET_COOKIE, limparCookie(ACCESS_TOKEN_COOKIE).toString())
				.header(HttpHeaders.SET_COOKIE, limparCookie(REFRESH_TOKEN_COOKIE).toString())
				.build();
	}
	...
	private ResponseEntity<AuthResponseDTO> respostaAutenticada(String mensagem, AuthResult result) {
		return ResponseEntity.ok()
				.header(HttpHeaders.SET_COOKIE, accessTokenCookie(result.tokens().accessToken()).toString())
				.header(HttpHeaders.SET_COOKIE, refreshTokenCookie(result.tokens().refreshToken()).toString())
				.body(new AuthResponseDTO(mensagem, UsuarioResponseDTO.from(result.usuario())));
	}
